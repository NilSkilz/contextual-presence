  ...
  >
    ...
    <AccordionItem>
      ...
      <AccordionBody accordionId="1">
        <div className="mb-3">
            ...
            <input className="form-control" id="id" type="text" placeholder="living_room" value={room._id} onChange={(({target})=> {
                setRoom({...room, _id: target.value});
            })}/>
        </div>
        <div className="mb-3">
            <label className="form-label" >Name</label>
            <input className="form-control" id="name" type="text" placeholder="Living Room" value={room.name} onChange={(({target})=> {
                setRoom({...room, name: target.value});
            })}/>
        </div>
        <div className="mb-3">
            <label className="form-label" >Floor</label>
            <select class="form-select" aria-label="Default select example" value={room.floor_id} onChange={(({target}) => {
    ...
    <AccordionItem>
      ...
      <AccordionBody accordionId="2">
        <div className="mb-3">
            {room.portals && room.portals.map((portal) => 
                <div className='row'>
                    <div className="mb-3 col-6">
                        <label className="form-label" >To</label>
                        <select class="form-select" aria-label="Default select example" value={portal.room_id} disabled={isEdit} onChange={(({target}) => {
                            // setRoom({...room, portals});
                        })}>
                            {rooms.map(room => {
                    ...
                    <div className="mb-3 col-6">
                        ...
                        <select class="form-select" aria-label="Default select example" value={portal.room_id} onChange={(({target}) => {
                            // setRoom({...room, portals});
                        })}>
                            {rooms.map(room => {
                                return <option value={room._id}>{room.name}</option>
        ...
        {isEdit ? (
          <div>
            <Button
              ...
              onClick={() => {
                if (isEdit) {
                  axios.delete(`/api/rooms/${roomToEdit._id}`, room).then(() => {
                    setIsOpen(false);
                  });
                }
              }}
